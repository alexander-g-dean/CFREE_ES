...
#pragma data_alignment=0x100 // align buffer to 256 byte boundaries, req. by DTC hardware
...
void main(void)
{
  ...
  LCDInit();    
	...
	while (1) {
		ADC_done = 0;
		R_DTCD0_Start(); // start DTC
		R_COMP0_Start();
		R_ADC_Start();   // Let ADC start waiting for ELC trigger

		// wait for all conversions and transfers
		while (!ADC_done)
			;
