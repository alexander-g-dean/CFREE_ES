...
extern float Energy[NUM_CHANNELS/2];
...
void LCDUpdate(uint8_t mode) {
	...
	case 1: // Power Display
		...
		break;
	case 2: // Energy & Power Display
		for (i=0; i<NUM_CHANNELS/2; i++) {
				LCDPrintf(i*4, 0,   "%d  %6.3f V", i+1, Voltage[i]);
				LCDPrintf(i*4+1, 0, "   %6.3f mA", 1000*Current[i]);

				//				LCDPrintf(i*4+1, 0, "  %8.6f A", Current[i]);

				LCDPrintf(i*4+2, 0, "  %7.3f mW", 1000*Power[i]);
				LCDPrintf(i*4+3, 0, "  %7.3f J", Energy[i]);
