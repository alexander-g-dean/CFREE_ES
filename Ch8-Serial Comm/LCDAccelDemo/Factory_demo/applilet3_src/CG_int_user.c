...
__interrupt void MD_INTP2(void)
{
    ...
    /* Disable the button interrupt for debounce */
        INTP2_Disable();
        
        /* Are we muted and we want to turn on the speaker? */
        if (mute == 1)
            mute = -1;  /* Request mute be turned off */
        
        /* Are we unmuted and we want to turn off the speaker? */
        if (mute == 0)
            mute = 2;   /* Request mute be enabled */
